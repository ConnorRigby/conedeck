...
config :nerves_ssh,
  authorized_keys: extra_keys ++ Enum.map(keys, &File.read!/1)
...
config :mdns_lite,
  ...
  # is "conedeck-<4 digit serial#>.local".  mdns_lite also advertises
  # "conedeck.local" for convenience. If more than one Nerves device is on the
  # network, delete "nerves" from the list.

  host: [:hostname, "conedeck"],
  ttl: 120,
...
config :conedeck, :viewport, %{
  name: :main_viewport,
  # default_scene: {Conedeck.Scene.SysInfo, nil},
  default_scene: {Conedeck.Scene.Splash, Conedeck.Scene.SysInfo},
  size: {800, 480},
  opts: [scale: 1.0],
  drivers: [
    %{
